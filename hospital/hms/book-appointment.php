...
if (isset($_POST['submit'])) {
    ...
    if ($specilization === '' || $doctorid === 0 || $userid === 0 || $appdate === '' || $time === '') {
        echo "<script>alert('Please fill in all required fields.');</script>";
    } elseif (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $appdate)) {
        ...
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, 'siisssi i', $specilization, $doctorid, $userid, $fees, $appdate, $time, $userstatus, $docstatus);
            if (mysqli_stmt_execute($stmt)) {
                echo "<script>alert('Your appointment successfully booked');</script>";
            } else {
                error_log('HMS book-appointment insert failed: ' . mysqli_stmt_error($stmt));
                echo "<script>alert('Booking failed. Please try again.');</script>";
            }
            mysqli_stmt_close($stmt);
        } else {
            error_log('HMS book-appointment prepare failed: ' . mysqli_error($con));
